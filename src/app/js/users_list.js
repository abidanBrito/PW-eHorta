...
let ViewUsersList = {
    ...
    represent: function(data) {
        ...
        data.forEach((user) => {
            ...
            this.table.innerHTML += "<tr class='content_row'>" + '<td  data-label="Empresa">' + "<input type='checkbox' name='checkUsers' style='margin:5px; class='nocheckbox'' value='checkbox'><a class='links_usuarios' 'id='user_" + `${user.id}` + "'>" + `${user.name}` + '</td><td></a>' + "<a class='edit-button' href='javascript:openUserForm()' title='Editar' margin='50px'><img src='img/edit.PNG' width='30' height='30'/></a></td></tr>"
...
function showCheckboxAdminClients() {
    ...
    document.getElementById("trashUsers").style.display = "block";
};
