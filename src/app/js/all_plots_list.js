...
let ViewAllPlotsList = {
    ...
    represent: function(data) {
        ...
        data.forEach((plot) => {
            ...
            this.text.innerHTML += "<tr class='content_row'>" + '<td  data-label="Empresa">' + "<input type='checkbox' style='margin:5px; class='nocheckbox' name='checkPlots' value='" + `${plot.id}` + "'><a class='links_usuarios' 'id='user_" + `${plot.id}` + "'>" + `${plot.name}` + '</td><td></a>' + "<a class='edit-button' href='javascript:openFieldForm()' title='Editar' margin='50px'><img src='img/edit.PNG' width='30' height='30' class='NoImage'/></a></td></tr>"
...
function showCheckboxAdminAllPlots() {
    ...
    document.getElementById("trashPlots").style.display = "block";
};
